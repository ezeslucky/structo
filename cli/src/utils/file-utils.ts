...
export function defaultResourcePath(
  ...
  return path.join(
    context.config.defaultStructoDir,
...
export function deleteFile(context: StructoContext, srcDirFilePath: string) {
  const path = makeFilePath(context, srcDirFilePath);
  deleteFileBuffered(path);
}

export function fileExists(context: StructoContext, srcDirFilePath: string) {
  return existsBuffered(makeFilePath(context, srcDirFilePath));
}

export function makeFilePath(context: StructoContext, filePath: string) {
...
export function findSrcDirPath(
  ...
  if (!path.isAbsolute(absoluteSrcDir)) {
    throw new HandledError("Cannot find srcDir. Please check structo.json.");
  }
  ...
  if (existsBuffered(path.join(absoluteSrcDir, expectedPath))) {
    ...
    throw new HandledError(
      `Cannot find expected file at ${expectedPath}, and found multiple possible matching files ${baseNameToFiles[fileName]}.  Please update structo.config with the real location for ${fileName}.`
    );
  }
}
 ...
 */
function getAllPaths(context: StructoContext): BundleKeyPair[] {
  ...
  pushPath(config, "defaultStructoDir");
 ...
 */
export function assertAllPathsInRootDir(context: StructoContext) {
  // Do not run this check when running in StructoLoader environment
  if (process.env.STRUCTO_LOADER) {
    return;
  }
