...
//@ts-ignore
import { initStructo } from "../actions/init";
import { StructoApi } from "../api";
...
function createStructoLock(): StructoLock {
  return {
    projects: [],
...
export function readLock(lockFile: string): StructoLock {
  if (!existsBuffered(lockFile)) {
    return createStructoLock();
  }
  try {
    const result = JSON.parse(readFileText(lockFile!)) as StructoLock;
    return {
      ...result,
    };
  } catch (e) {
...
function removeMissingFilesFromLock(
  context: StructoContext,
  config: StructoConfig,
  lock: StructoLock
) {
  ...
  if (!baseNameToFiles[fileName]) {
    ...
    if (answer === "No") {
      throw new HandledError(
        "Please add this file or update your structo.json by removing or changing this path and try again."
      );
    }
    return undefined;
  }
...
export async function getContext(
  ...
  if (!configFile) {
    //@ts-ignore
    await maybeRunStructoInit(args, "structo.json", enableSkipAuth);
    configFile = findConfigFile(args.baseDir, { traverseParents: true });
    if (!configFile) {
      const err = new HandledError(
        "No structo.json file found. Please run `structo init` first."
      );
      throw err;
    }
  }
...
//@ts-ignore
  if (await maybeRunStructoInit(args, ".structo.auth")) {
    return ensure(await getCurrentAuth());
  }

  // Could not find the authentication credentials and the user
  // declined to run structo init.
  throw new HandledError("Could not authenticate.");
}

async function maybeRunStructoInit(
  args: CommonArgs,
  missingFile: string,
  enableSkipAuth?: boolean
): Promise<boolean> {
  if (!process.env.QUIET) {
    logger.info(`No ${missingFile} file found. Initializing structo...`);
  }
...
export interface Metadata {
  ...
  source?: "cli" | "loader" | "create-structo-app" | "structo-action";
...
export function generateMetadata(
  ...
  const fromEnv = process.env.STRUCTO_METADATA;
...
export function setMetadataEnv(metadata: Metadata): void {
  const fromEnv = process.env.STRUCTO_METADATA
    ? querystring.decode(process.env.STRUCTO_METADATA)
    : {};
  const env = { ...fromEnv };
  ...
  process.env.STRUCTO_METADATA = querystring.encode(env);
}
