...
export interface CodeComponentMeta<P> {
   ...
   * component. Optional: if not specified, Structo will expect it to be
  ...
  /**
   * The prop that receives and forwards a React `ref`. Structo only uses `ref`
   * to interact with components, so it's not used in the generated code.
   * Optional: If not provided, the usual `ref` is used.
   */
  refProp?: string;
  /**
   * Default styles to start with when instantiating the component in Structo.
   */
...
declare global {
  interface Window {
    __StructoComponentRegistry: ComponentRegistration[];
  }
}

if (root.__StructoComponentRegistry == null) {
  root.__StructoComponentRegistry = [];
}
...
export default function registerComponent<T extends React.ComponentType<any>>(
  ...
  if (
    root.__StructoComponentRegistry.some(
  ...
  root.__StructoComponentRegistry.push({ component, meta });
}
