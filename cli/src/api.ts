...
export class StructoApi {
  ...
  async genLocalizationStrings(
    ...
    const result = await this.get(
      ...
      undefined,
      {
        "x-structo-api-project-tokens": projectIdsAndTokens
          .map(
  ...
  private makeErrorMessage(error: AxiosError) {
    ...
    if (response.status === 403) {
      return `Incorrect Structo credentials; please check your .structo.auth file or your project API tokens.`;
    }
  ...
  private makeHeaders() {
    const headers: Record<string, string> = {
      "x-structo-api-user": this.auth.user,
      "x-structo-api-token": this.auth.token,
    };
  ...
  private get codegenHost() {
    if (!this.auth.host || this.auth.host === DEFAULT_HOST) {
      return "https://codegen.structo.app";
    } else {
      return this.auth.host;
    }
  }
