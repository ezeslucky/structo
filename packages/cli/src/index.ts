...
import { getYargsOption, InitArgs, initStructo } from "./actions/init";
import {
...
yargs
  ...
  .option("auth", {
    describe:
      "Structo auth file to use; by default, uses ~/.structo.auth, or the first .structo.auth file found in current and parent directories",
  })
  .option("config", {
    describe:
      "Structo config file to use; by default, uses the first structo.json file found in the current or parent directories",
  })
  .option("yes", {
    type: "boolean",
  ...
  .command<InitArgs>(
    "init",
    "Initializes Structo for a project.",
    (yags) => {
      yags
        .option("enable-skip-auth", {
          describe: "Enable skipping auth, just initialize a structo.json",
          type: "boolean",
          default: false,
        })
        .option("host", getYargsOption("host", "https://studio.structo.app"))
        ...
        .option("structo-dir", getYargsOption("structoDir"))
    ...
    (argv) => handleError(initStructo(argv))
  )
  .command<auth.AuthArgs>(
    "auth",
    "Authenticates you to structo.",
    (yags) => {
      yags
        .option("host", {
          describe: "Structo host to use",
          type: "string",
          default: "https://studio.structo.app",
        })
        .option("check", {
          alias: "c",
  ...
  .command<SyncArgs>(
    "sync",
    "Syncs designs from Structo to local files.",
  ...
  .command<FixImportsArgs>(
    "fix-imports",
    "Fixes import paths after you've moved around Structo blackbox files",
    (yags) =>
  ...
  .command<InfoArgs>(
    ...
    (yags) =>
      yags
        ...
        .option("projects", {
          alias: "p",
          describe: "ID of structo project to check",
          type: "array",
          default: [],
        })
        .option("json", {
  ...
  .command<UploadBundleArgs>(
    ...
    (yargs) =>
      yargs
        .option("project", {
          alias: "p",
          describe: "ID of Structo project to upload the bundle to.",
          type: "string",
        })
        .option("bundleName", {
    ...
    (yargs) =>
      yargs
        .option("projects", {
          alias: "p",
          describe:
            "One or more projects to generate localization strings, separated by comma. Version constraints can be specified using @. Example: projectid, projectid@>=version. If not specified, will be derived from structo.json if you are using codegen.",
          type: "array",
        })
        .option("host", {
          describe: "Structo host to use",
...
function configureSyncArgs(
  ...
  let args = yags
    ...
    .option("ignore-post-sync", {
      type: "boolean",
      describe: "Ignore post-sync commands in structo.json",
      default: false,
    })
    .option("skip-upgrade-check", {
      type: "boolean",
      describe: "Skip upgrade check for Structo dependencies",
      default: false,
    })
    .option("metadata", {
      type: "string",
