...
export async function ensureImageAssetContents(bundles: ImageBundle[]) {
  ...
  const needsFetching = bundles.filter((b) =>
    b.blob.startsWith("https://site-assets.structo.app/")
  );
...
function getImagePublicUrl(context: StructoContext, asset: ImageConfig) {
  return (
...
const RE_ASSETTSXREF_ALL = /Structo_Image_([^\)\s]+)__Ref/g;
