...
import { StructoConfig } from "./config-utils";
...
export function checkEngineStrict(): boolean {
  ...
  if (!!minNodeVersion && !semver.satisfies(process.version, minNodeVersion)) {
    logger.warn(`Structo only works on Node ${minNodeVersion}`);
    return false;
  }
  return true;
}
...
export function detectPackageManager(config: StructoConfig, baseDir: string) {
