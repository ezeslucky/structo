...
async function ensureRequiredPackages(
  ...
  if (
    !reactWebVersion ||
    semver.gt(requireds["@structo/react-web"], reactWebVersion)
  ) {
    await confirmInstall(
      "@structo/react-web",
      requireds["@structo/react-web"],
      reactWebVersion,
  ...
  if (context.config.code.reactRuntime === "automatic") {
    ...
    const runtimeVersion = findInstalledVersion(
      context.config,
      baseDir,
      "@structo/react-web-runtime"
    );
    if (
      !runtimeVersion ||
      semver.gt(requireds["@structo/react-web-runtime"], runtimeVersion)
    ) {
      await confirmInstall(
        "@structo/react-web-runtime",
        requireds["@structo/react-web-runtime"],
        runtimeVersion,
        { global: false, dev: false }
      );
    }
...
export async function sync(
  ...
  if (projectSyncParams.some((p) => !p.projectApiToken)) {
    try {
      ...
      if ((e as any).message.includes("Unable to authenticate Structo")) {
        const configFileName = CONFIG_FILE_NAME;
        throw new HandledError(
          `Unable to authenticate Structo. Please run 'structo auth' or check the projectApiTokens in your ${configFileName}, and try again.`
        );
      }
    }
  }
...
function fixFileExtension(context: StructoContext) {
  const cssExt =
