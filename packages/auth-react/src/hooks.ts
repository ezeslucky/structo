import {
  getStructoAppUser,
  getStructoAppUserFromToken,
  StructoUser,
} from "@structoapp/auth-api";
import { useMutableStructoQueryData } from "@structoapp/query";

interface StructoAuthData {
...
async function handleCallback(opts: {
  ...
  state: string;
  codeVerifier: string;
}): Promise<StructoAuthData | undefined> {
  ...
  const result = await getStructoAppUser({
    host,
    appId,
    code,
    codeVerifier,
...
async function checkAlreadyLoggedUser(opts: {
  ...
  const { user, error } = await getStructoAppUserFromToken({
    host,
    token,
  });
...
/**
 * Handles the authentication flow for Structo Auth and returns the user and token
 */
export function useStructoAuth(opts: { host?: string; appId?: string }) {
  const { host, appId } = opts;
  const authKey = `$csq$Structo-auth-${appId}`;
  const { data: userData, isLoading } = useMutableStructoQueryData(
    authKey,
    async (): Promise<StructoAuthData> => {
      if (!appId || !isBrowser) {
        return { user: null, token: null };
      }
