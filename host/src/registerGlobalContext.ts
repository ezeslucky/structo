...
declare global {
  interface Window {
    __StructoContextRegistry: GlobalContextRegistration[];
  }
}

if (root.__StructoContextRegistry == null) {
  root.__StructoContextRegistry = [];
}
...
>(component: T, meta: GlobalContextMeta<React.ComponentProps<T>>) {
  ...
  if (
    root.__StructoContextRegistry.some(
  ...
  root.__StructoContextRegistry.push({ component, meta });
}
