...
import { CONFIG_FILE_NAME, StructoContext } from "./config-utils";
...
async function checkProjectMeta(
  ...
  const checkVersionLock = async (): Promise<boolean> => {
    ...
    if (semver.lt(newVersion, versionOnDisk)) {
      meta === root
        ? logger.warn(
            `The local version of '${projectName}' (${versionOnDisk}) is higher than requested version @${newVersion}. Structo does not support downgrading a project. You should consider updating the version range in ${CONFIG_FILE_NAME}.`
          )
        : logger.warn(
            `'${root.projectName}' uses '${projectName}'@${newVersion}, but your code has '${projectName}'@${versionOnDisk}. You should consider upgrading this dependency in Structo Studio.`
          );
      return false;
    }
    ...
    throw new Error(
      `Error comparing version=${newVersion} with the version found in structo.lock (${versionOnDisk}) for '${projectName}'`
    );
  };
  ...
  const checkIndirect = async (): Promise<boolean> => {
    ...
    if (configIndirect && !indirect) {
      logger.warn(
        `'${projectName}' was synced indirectly before, but a direct sync was requested. If it has page components, they will be synced and saved to structo.json.`
      );
      return await confirmWithUser(
  ...
  if (!checkedVersion && isOnDiskCodeInvalid) {
    ...
    const versionOnDisk = projectLock?.version;
    logger.warn(
      `Project '${projectName}' was synced by an incompatible version of Structo Codegen. Syncing again on the same version ${projectName}@${versionOnDisk}`
    );
