...
export async function initStructo(
  ...
  if (configFile && existsBuffered(configFile)) {
    logger.error(
      "You already have a structo.json file! Please either delete or edit it directly."
    );
    return;
  }
  ...
  if (!process.env.QUIET) {
    logger.info("Successfully created structo.json.\n");
  }

  const answer = await confirmWithUser(
    "@structo/react-web is a small runtime required by Structo-generated code.\n  Do you want to add them now?",
    opts.yes
  );
  if (answer) {
    installUpgrade(initConfig, "@structo/react-web", opts.baseDir);
  }
}

function createInitConfig(opts: Omit<InitArgs, "baseDir">): StructoConfig {
  return fillDefaults({
    srcDir: opts.srcDir,
    defaultStructoDir: opts.structoDir,
...
async function deriveInitAnswers(
  ...
  const structoRootDir = opts.config ? path.dirname(opts.config) : opts.baseDir;
  ...
  const deriver = isNext
    ? getNextDefaults(structoRootDir, isNextAppDir)
    : isGatsby
    ? getGatsbyDefaults(structoRootDir)
    : isTanStack
    ? getTanStackDefaults(structoRootDir)
    : isCra
    ? getCraDefaults(structoRootDir)
    : getGenericDefaults(structoRootDir);
  ...
  const answers: Omit<InitArgs, "baseDir"> = {
    ...
    structoDir: getDefaultAnswer(
      "structoDir",
      DEFAULT_CONFIG.defaultStructoDir
    ) as any,
  ...
  logger.info(
    chalk.bold(
      "Structo Express Setup -- Here are the default settings we recommend:\n"
    )
  );
  ...
  async function performAsks(express: boolean) {
    ...
    await maybePrompt({
      name: "structoDir",
      message: `${getInitArgsQuestion("structoDir")} (This is relative to "${
        answers.srcDir
      }")\n>`,
    });
    ...
    await maybePrompt({
      ...
      choices: () => [
        {
          name: `CSS modules, imported as "import sty from './structo.module.css'"`,
          value: "css-modules",
        },
        {
          name: `Plain CSS stylesheets, imported as "import './structo.css'"`,
          value: "css",
        },
      ],
    });
...
function getNextDefaults(
  structoRootDir: string,
  appDir: boolean
): DefaultDeriver {
  const projectRootDir = findPackageJsonDir(structoRootDir) ?? structoRootDir;
  return {
    ...
    pagesDir: (srcDir: string) =>
      path.relative(
        path.join(structoRootDir, srcDir),
...
function getGatsbyDefaults(structoRootDir: string): DefaultDeriver {
  const projectRootDir = findPackageJsonDir(structoRootDir) ?? structoRootDir;
  return {
    srcDir: path.relative(
      structoRootDir,
    ...
    pagesDir: (srcDir: string) => {
      const absSrcDir = path.join(structoRootDir, srcDir);
...
function getTanStackDefaults(structoRootDir: string): DefaultDeriver {
  const projectRootDir = findPackageJsonDir(structoRootDir) ?? structoRootDir;
  return {
    srcDir: path.relative(
      structoRootDir,
...
function getCraDefaults(structoRootDir: string): DefaultDeriver {
  const projectRootDir = findPackageJsonDir(structoRootDir) ?? structoRootDir;
  return {
    srcDir: path.relative(
      structoRootDir,
...
function getGenericDefaults(structoRootDir: string): DefaultDeriver {
  const projectRootDir = findPackageJsonDir(structoRootDir) ?? structoRootDir;
  const srcDir = existsBuffered(path.join(projectRootDir, "src"))
    ? path.join(projectRootDir, "src", "components")
    : path.join(projectRootDir, "components");
  return {
    srcDir: path.relative(structoRootDir, srcDir),
    alwaysDerived: [],
  };
}
...
const INIT_ARGS_DESCRIPTION: {
  ...
  host: {
    shortDescription: "Structo host to use",
  },
  platform: {
  ...
  codeLang: {
    ...
    question: `What target language should Structo generate code in?`,
    choices: ["js", "ts"],
  },
  codeScheme: {
  ...
  styleScheme: {
    ...
    question: "How should we generate css for Structo components?",
    choices: ["css", "css-modules"],
  },
  imagesScheme: {
    shortDescription: "Image scheme",
    longDescription: "How to reference used image files",
    question: "How should we reference image files used in Structo components?",
  ...
  structoDir: {
    shortDescription: "Structo-managed directory",
    longDescription:
      "Default directory to put Structo-managed files into; relative to src-dir",
    question:
      "What directory should Structo-managed files (that you should not edit) be put into?",
  },
  pagesDir: {
